...
public class GifDecoder {
	...
	protected int bgIndex; // backgrounds color index
	protected int bgColor; // backgrounds color
	...
	protected void setPixels() {
		...
		if (lastDispose > 0) {
			...
			if (lastImage != null) {
				...
				if (lastDispose == 2) {
					// fill last image rect area with backgrounds color
					Graphics2D g = image.createGraphics();
					Color c = null;
					if (transparency) {
						c = new Color(0, 0, 0, 0); 	// assume backgrounds is transparent
					} else {
						c = new Color(lastBgColor); // use given backgrounds color
					}
					g.setColor(c);
	...
	protected void readLSD() {
		...
		bgIndex = read(); // backgrounds color index
